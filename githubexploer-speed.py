...
class LoginWindow(tk.Toplevel):
    def __init__(self, parent, config):
        ...
        ttk.Label(self, text="请输入 GitHub Personal Access Token (需开启 workflow 权限):").pack(pady=10)
...
class MainWindow(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # 左侧 notebook
        self.left_notebook = ttk.Notebook(main_paned)
        main_paned.add(self.left_notebook, weight=0)

        # 绑定选项卡切换事件，用于自动加载 Actions 数据
        self.left_notebook.bind("<<NotebookTabChanged>>", self.on_tab_changed)

        # ---------- 标签1：仓库列表 ----------
        repo_tab = ttk.Frame(self.left_notebook, width=int(280 * self.font_scale))
        self.left_notebook.add(repo_tab, text="仓库列表")
        ...
        # ---------- 标签2：目录树 ----------
        tree_tab = ttk.Frame(self.left_notebook, width=int(280 * self.font_scale))
        self.left_notebook.add(tree_tab, text="目录树")
        ...
        # ---------- 标签3：Actions ----------
        self.actions_tab = ttk.Frame(self.left_notebook, width=int(280 * self.font_scale))
        self.left_notebook.add(self.actions_tab, text="Actions")

        # 上半部分：Workflow 选择
        wf_top = ttk.Frame(self.actions_tab)
        ...
        self.trigger_btn = ttk.Button(wf_top, text="触发", command=self.trigger_selected_workflow)
        self.trigger_btn.pack(side=tk.LEFT, padx=2)

        # 状态筛选
        filter_frame = ttk.Frame(self.actions_tab)
        ...
        # Runs 列表
        self.runs_tree = ttk.Treeview(self.actions_tab, columns=("id", "status", "time", "action"), show="headings",
                                      selectmode="browse")
    ...
    def on_tab_changed(self, event):
        ...
        tab_text = self.left_notebook.tab(current_tab, "text")
        if tab_text == "Actions" and self.current_repo:
            # 如果 workflows 列表为空，自动加载
            if not self.workflows:
                self.load_workflows_sync()
            else:
                # 已有 workflows，刷新一下 runs 以保持最新
    ...
    def load_workflows_sync(self):
        ...
        try:
            ...
            if wf_names:
                ...
                messagebox.showinfo("提示", "当前仓库没有可用的 Workflows。")
    ...
    def _refresh_runs_list(self):
        self.runs_tree.configure(displaycolumns=())
        self.runs_tree.delete(*self.runs_tree.get_children())
    ...
    def trigger_selected_workflow(self):
        # 防重复触发
        if self.triggering:
            messagebox.showinfo("提示", "正在触发中，请稍后...")
            return
        ...
        def do_trigger():
            try:
                ...
                self.set_status("触发成功，刷新 runs...", cursor="")
                # 延迟后刷新 runs 列表，并重新启用按钮
                self.after(2000, self._after_trigger_refresh)
            except Exception as e:
    ...
    def _after_trigger_refresh(self):
        """触发后刷新 runs 并恢复按钮"""
        self.load_runs_sync()
        self.triggering = False
        self.trigger_btn.config(state=tk.NORMAL)
    ...
    def on_run_select(self, event):
        ...
        if not sel:
            return
        item = sel[0]
        values = self.runs_tree.item(item, "values")
        if not values:
            return
        run_id = values[0]
    ...
    def on_run_click(self, event):
        ...
        if region != "cell" or col != "#4":  # action 列是第4列
            return
        ...
        if not values:
            return
        action_text = values[3]   # action 列内容
        if action_text != "下载":
            return
        run_id = int(values[0])
    ...
    def on_repo_select(self, event):
        ...
        # 重置 workflows 相关状态，因为换了仓库
        self.workflows = []
        self.current_workflow_id = None
        self.wf_combo.set('')
        self.wf_combo['values'] = []
        self.runs_tree.delete(*self.runs_tree.get_children())
        self.clear_content()
        self.load_repo_tree_sync(owner, repo_name, branch)
        # 自动切换到目录树标签
        self.left_notebook.select(self.left_notebook.tabs()[1])  # 索引1是目录树
